...
class Command(Protocol):
	...
	def dataReceived(self, data):
		if self.firstConnected==0:
			newBlueGhostRect = pickle.loads(data)
			gs.blueGhost.rect = newBlueGhostRect
...
class blueGhost(pygame.sprite.Sprite):
	...
	def __init__(self, gs=None):
		...
		self.count = 0
		self.l = ["blueGhost",self.rect,self.image]
	...
	def validMove(self, x, y):
		...
		if x>636:
			...
			#print "this one"
			return 0
		elif x>476 and x<532 and y>333 and y<450:
			#print "that one"
			return 0
...
if __name__ == '__main__':
	...
	reactor.connectTCP('localhost', 32000, commandFactory) #command port number
