...
class orangeGhost(pygame.sprite.Sprite):
	...
	def __init__(self, gs=None):
		...
		self.count = 0
		self.l = ["orangeGhost",self.rect,self.image]
		...
		if self.rect.x>365 and self.rect.x<416 and self.rect.y < 380 and self.rect.y>272:
			...
			if (keycode == K_RIGHT):
				...
				if self.validMove(self.rect.x+4, self.rect.y):
					...
					self.l = ["orangeGhost", self.rect,0]
			...
			if (keycode == K_LEFT):
				...
				if self.validMove(self.rect.x-4, self.rect.y):
					if self.count<1:
						...
						self.count = 0
					self.rect = self.rect.move(-4,0)
					self.l = ["orangeGhost", self.rect,1]
			...
			if (keycode == K_DOWN):
				...
				if self.validMove(self.rect.x, self.rect.y+4):
					...
					self.l = ["orangeGhost", self.rect,2]
			...
			if (keycode == K_UP):
				...
				if self.validMove(self.rect.x, self.rect.y-4):
					if self.count<1:
						...
						self.count = 0
					self.rect = self.rect.move(0,-4)
					self.l = ["orangeGhost", self.rect,3]
	...
	def task(self):
		...
		for event in pygame.event.get():
			if (event.type == KEYDOWN):
				if self.orangeGhost.alive != 0:
					self.orangeGhost.move(event.key)
		...
		#	if (self.orangeGhost.alive == 0 and self.redGhost.alive == 0):
		#	self.deathCounter += 1
